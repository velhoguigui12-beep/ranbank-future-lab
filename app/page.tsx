...
const presentationSlides = [
  { chapter: "1 · IA E BIG DATA", icon: "IA", title: "Dos dados bancários à decisão", text: "O Big Data reúne milhares de movimentações, valores, horários e dispositivos. A IA encontra padrões nesses dados e aponta operações que merecem verificação.", talk: "Use a análise de fraude: Big Data organiza os sinais e a IA calcula o risco. A decisão importante continua sob supervisão humana.", points: ["Coletar dados", "Encontrar padrões", "Apoiar decisões"] },
  { chapter: "2 · IOT E SUSTENTABILIDADE", icon: "IoT", title: "Dispositivos que medem e ajudam a economizar", text: "Celulares, caixas eletrônicos e sensores enviam informações em tempo real. Esses dados permitem acompanhar consumo de energia, equipamentos e emissões do banco.", talk: "Abra Dispositivos e depois Sustentabilidade. Mostre que IoT mede o ambiente e Green IT transforma a medição em redução de desperdício.", points: ["Conectar", "Medir", "Otimizar"] },
  { chapter: "3 · VR E RA", icon: "XR", title: "Treinamento virtual e orientação no mundo real", text: "A realidade virtual cria um ambiente completo para treinamento. A realidade aumentada adiciona instruções digitais ao ambiente real do cliente.", talk: "Compare: VR pode treinar funcionários contra golpes; RA pode orientar uma operação no aplicativo ou em um caixa eletrônico.", points: ["VR: imersão", "RA: sobreposição", "Experiência"] },
  { chapter: "4 · COMPUTAÇÃO EM NUVEM", icon: "☁", title: "O banco disponível em qualquer lugar", text: "A nuvem hospeda sistemas e dados em servidores acessados pela internet, permitindo cópias, recuperação de falhas e aumento de capacidade quando necessário.", talk: "O próprio Ranbank publicado é um exemplo: o site e a API Java continuam acessíveis mesmo com o computador de desenvolvimento desligado.", points: ["Disponibilidade", "Recuperação", "Escalabilidade"] },
  { chapter: "5 · COMPARAÇÃO", icon: "≋", title: "Não existe uma tecnologia melhor para tudo", text: "Cada tecnologia resolve um tipo de problema. A escolha muda conforme o objetivo do banco: segurança, eficiência, escala, custo ou experiência do cliente.", talk: "Abra o comparador e troque o objetivo. Mostre que benefícios, custos e limitações precisam ser avaliados juntos.", points: ["Objetivo", "Benefícios", "Limitações"] },
  { chapter: "6 · ROBÓTICA", icon: "R2", title: "Sensores, decisões e movimentos", text: "A robótica combina sensores, software e componentes físicos para executar tarefas. Em um banco, pode orientar clientes e apoiar rotinas de atendimento.", talk: "Abra a missão do robô e destaque que tarefas repetitivas podem ser automatizadas, mas decisões sensíveis exigem supervisão humana.", points: ["Perceber", "Processar", "Agir"] },
];
        ...
        <section className="login-panel">
          <form className={`login-card ${authMode === "create" ? "signup-card" : ""}`} onSubmit={authMode === "login" ? login : createDemoAccount}>
            ...
            {authMode === "login" ? <>
              ...
              <label>PIN de acesso<input className="login-pin-input" name="ranbank-access-pin" type="password" value={loginPin} onChange={(event) => setLoginPin(event.target.value.replace(/\D/g, "").slice(0,4))} autoComplete="off" inputMode="numeric" maxLength={4} placeholder="••••" aria-label="PIN de quatro dígitos"/></label>
              <div className="pin-dots" aria-hidden="true">{[0,1,2,3].map((index) => <i key={index} className={index < loginPin.length ? "filled" : ""}/>)}</div>
              <div className="numeric-keypad" aria-label="Teclado numérico">{[1,2,3,4,5,6,7,8,9].map((digit) => <button type="button" key={digit} onClick={() => appendLoginDigit(String(digit))}>{digit}</button>)}<button type="button" className="biometric-key" disabled aria-label="Passkey ainda não cadastrada">◎</button><button type="button" onClick={() => appendLoginDigit("0")}>0</button><button type="button" className="erase-key" onClick={() => setLoginPin((current) => current.slice(0,-1))} aria-label="Apagar último dígito">⌫</button></div>
            ...
            {authMode === "login" && <><div className="biometric-login" role="note"><span>◎</span><div><strong>Biometria via passkey ainda não cadastrada</strong><small>Se o iPhone pedir Face ID, ele está liberando uma senha salva no aparelho. O RanBank não recebe sua imagem facial.</small></div></div><div className="demo-credentials"><b>ACESSO PARA APRESENTAÇÃO</b><span>CPF 123.456.789-09</span><span>PIN 2580</span></div></>}
