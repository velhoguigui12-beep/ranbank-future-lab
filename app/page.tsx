...
const technologies = [
  ...
  { icon: "♧", title: "Sustentabilidade", text: "Otimize energia, emissões e capacidade da infraestrutura." },
...
const presentationSlides = [
  ...
  { chapter: "6 · ECOSSISTEMA", icon: "OF", title: "Dados sob controle da cliente", text: "Open Finance reúne instituições diferentes por meio de consentimentos claros, com prazo e possibilidade de revogação.", talk: "Abra o painel de Open Finance, conecte uma instituição e mostre como o total consolidado muda.", points: ["Consentimento", "APIs", "Portabilidade"] },
  { chapter: "7 · JORNADA", icon: "360", title: "Tecnologias trabalhando juntas", text: "Uma suspeita percorre IoT, Big Data, IA, automação, nuvem e auditoria antes de chegar à decisão humana.", talk: "Abra a Jornada antifraude e percorra as seis etapas, destacando que a pessoa continua no controle.", points: ["Contexto", "Orquestração", "Decisão humana"] },
  { chapter: "8 · CONCLUSÃO", icon: "✓", title: "Tecnologia precisa de propósito", text: "Não existe uma tecnologia melhor para tudo. A escolha depende do problema, do custo, dos riscos e das pessoas envolvidas.", talk: "Finalize abrindo o comparador e convide o público a escolher um objetivo.", points: ["Contexto", "Responsabilidade", "Impacto"] },
];
...
function answerLocally(message: string) {
  ...
  if (has("energia", "sustentavel", "sustentabilidade", "green it")) return { topic: "Sustentabilidade", text: "Green IT busca reduzir energia, emissões e desperdício por meio de software, infraestrutura eficiente e melhor uso da nuvem." };
    ...
    <main className="bank-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-status">
          ...
          <div><strong>{backendOnline ? "Java conectado" : "Modo demonstração"}</strong><small>{backendOnline ? "API local ativa" : "Dados locais seguros"}</small></div>
        </div>
      </aside>
      ...
      {pixOpen && <div className="modal-backdrop" role="presentation" onMouseDown={() => setPixOpen(false)}><section className="pix-modal" role="dialog" aria-modal="true" aria-labelledby="pix-title" onMouseDown={(event) => event.stopPropagation()}><header><div><span>PIX DEMONSTRATIVO</span><h2 id="pix-title">Enviar um Pix</h2></div><button onClick={() => setPixOpen(false)} aria-label="Fechar">×</button></header><div className="education-note"><b>i</b><p><strong>Operação educacional</strong><br/>Nenhum dinheiro real será movimentado.</p></div><form onSubmit={sendPix}><label>Chave Pix<input value={pixKey} onChange={(event) => setPixKey(event.target.value)} placeholder="CPF, celular, e-mail ou chave aleatória" required /><small className="field-help">Celular com DDD · CPF com 11 dígitos · chave aleatória no formato UUID</small></label><label>Valor disponível: {money.format(data.balance)}<input value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0,00" inputMode="decimal" required /></label>{pixStatus === "error" && <p className="form-error">{pixError}</p>}<button className="confirm-pix" disabled={pixStatus === "sending"}>{pixStatus === "sending" ? "Validando…" : pixStatus === "success" ? "Pix registrado ✓" : "Confirmar simulação"}</button></form></section></div>}
      ...
      {sustainabilityOpen && <div className="modal-backdrop" role="presentation" onMouseDown={() => setSustainabilityOpen(false)}><section className="sustainability-modal" role="dialog" aria-modal="true" aria-labelledby="sustainability-title" onMouseDown={(event) => event.stopPropagation()}><header><div><span>GREEN IT · EFICIÊNCIA ENERGÉTICA</span><h2 id="sustainability-title">Infraestrutura sustentável</h2></div><button onClick={() => setSustainabilityOpen(false)} aria-label="Fechar">×</button></header>{sustainabilityLoading && !sustainability ? <div className="analysis-loading"><i/><p>Medindo consumo…</p></div> : sustainability ? <><div className={`green-status ${sustainability.optimized ? "green-optimized" : ""}`}><div><span>Estado</span><strong>{sustainability.optimized ? "OTIMIZADO" : "MELHORIAS DISPONÍVEIS"}</strong></div><div><span>Economia simulada</span><strong>{sustainability.savingsPercent}%</strong></div><button disabled={sustainabilityLoading} onClick={() => requestSustainability(true)}>{sustainabilityLoading ? "Calculando…" : sustainability.optimized ? "Desfazer otimização" : "Otimizar infraestrutura"}</button></div><div className="green-metrics"><article><span>Consumo</span><strong>{sustainability.powerKw.toFixed(1)} kW</strong><small>Potência instantânea</small></article><article><span>Energia renovável</span><strong>{sustainability.renewablePercent}%</strong><small>Participação na matriz</small></article><article><span>Emissões</span><strong>{sustainability.carbonKgHour.toFixed(1)} kg</strong><small>CO₂ equivalente / hora</small></article><article><span>PUE</span><strong>{sustainability.pue.toFixed(2)}</strong><small>Eficiência do data center</small></article></div><div className="energy-sources"><div><span>COMPOSIÇÃO ENERGÉTICA</span><small>Dados fictícios para demonstração</small></div>{sustainability.sources.map((source) => <article key={source.name}><div><strong>{source.name}</strong><small>{source.type}</small></div><span><i style={{width:`${source.percentage}%`}}/></span><b>{source.percentage}%</b></article>)}</div><div className="green-actions">{sustainability.actions.map((action,index) => <span key={action}><b>{sustainability.optimized ? "✓" : "!"}</b>{action}</span>)}</div><p className="analytics-caption">Os valores são simulados. O módulo demonstra como software, nuvem e gestão energética podem reduzir desperdício e emissões.</p></> : <div className="analysis-error"><strong>Backend não disponível</strong><p>Reinicie o Spring Boot para carregar as métricas.</p></div>}</section></div>}
